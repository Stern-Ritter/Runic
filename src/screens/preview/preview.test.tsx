...
describe("Preview screen", () => {
  ...
  it("should render correct Text elements", () => {
    ...
    expect(wrapper.find('Text').findWhere((text) => 
      text.text() === 'Runic').exists()).toBeTruthy();
    expect(wrapper.find('Text').findWhere((text) => 
      text.text() === 'The best app for running.').exists()).toBeTruthy();
  });
  ...
  it("should render buttons with correct handlers", () => {
    ...
    const createAccountBtn = wrapper.find('ForwardRef')
      .findWhere((f) => f.text() === 'Регистрация').first();
    const loginBtn = wrapper
      .find('ForwardRef').findWhere((f) => f.text() === 'Войти').first();
