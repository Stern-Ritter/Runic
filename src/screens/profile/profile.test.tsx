...
const setSettingsFormValueSpy = jest
    .spyOn(profileActions, "setSettingsFormValue");
...
describe("Profile screen", () => {
  ...
  it(`should section names and labels`, () => {
    ...
    expect(wrapper.find('Text').findWhere((text) => 
        text.text() === 'Основные:').exists()).toBeTruthy(); 
    expect(wrapper.find('Text').findWhere((text) => 
        text.text() === 'Имя пользователя').exists()).toBeTruthy();
    expect(wrapper.find('Text').findWhere((text) => 
        text.text() === 'Цели на неделю:').exists()).toBeTruthy();
    expect(wrapper.find('Text').findWhere((text) => 
        text.text() === 'Километры').exists()).toBeTruthy();
    expect(wrapper.find('Text').findWhere((text) => 
        text.text() === 'Калории').exists()).toBeTruthy();
    });
    ...
    it(`should render inputs with correct props, values and handlers`, () => {
        ...
        const nameInput = wrapper.find("TextInput")
            .filterWhere((input) => input.props().placeholder === 'Ник');
        const distanceGoalInput = wrapper.find("TextInput")
            .filterWhere((input) => input.props().placeholder === 'Цель километров');
        const caloriesGoalInput = wrapper.find("TextInput")
        ...
        expect(distanceGoalInput.prop('value')).toBe(settings.settings.data.distanceGoal);

        expect(caloriesGoalInput.prop('keyboardType')).toBe('numeric');
        expect(caloriesGoalInput.prop('value')).toBe(settings.settings.data.caloriesGoal);
    
        nameInput.simulate('changeText', 'New nick');
        expect(setSettingsFormValueSpy).toHaveBeenCalledTimes(1);
        expect(setSettingsFormValueSpy).toHaveBeenLastCalledWith({field: "nickName", value: "New nick"});
    
        distanceGoalInput.simulate('changeText', '33');
        expect(setSettingsFormValueSpy).toHaveBeenCalledTimes(2);
        expect(setSettingsFormValueSpy).toHaveBeenLastCalledWith({field: "distanceGoal", value: 33});

        caloriesGoalInput.simulate('changeText', '1500');
        expect(setSettingsFormValueSpy).toHaveBeenCalledTimes(3);
        expect(setSettingsFormValueSpy).toHaveBeenLastCalledWith({field: "caloriesGoal", value: 1500});
      });
      ...
      it(`should render buttons with correct handlers`, async () => {
        ...
        const saveBtn = wrapper.find('ForwardRef')
          .findWhere((f) => f.text() === 'Сохранить').first();
        const logoutBtn = wrapper.find('ForwardRef')
          .findWhere((f) => f.text() === 'Сменить пользователя').first();
      ...
      it(`should render message about save settings error`, async () => {
        ...
        expect(wrapper.find('Text').findWhere((text) => 
          text.text() === `При обновлении настроек прозошла ошибка.\nПопробуйте позже.`)
            .exists()).toBeTruthy();
      });
});
